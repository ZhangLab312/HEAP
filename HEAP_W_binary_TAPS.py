...
class chromatin_encoding(nn.Module):
    def __init__(self, motiflen=9):
        ...
        self.trans1 = make_model(1, 64, 128,
                                 8)  # nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=64, nhead=8, dim_feedforward=128), num_layers=2)
        self.trans2 = make_model(1, 64, 128,
                                 8)  # (nn.TransformerEncoderLayer(d_model=64, nhead=8, dim_feedforward=128), num_layers=2)
        self.conv = freeze_conv(in_channels=12, out_channels=64, kernel_size=motiflen, padding='same')
        self.conv1 = freeze_conv(in_channels=64, out_channels=64, kernel_size=motiflen, padding='same')
        self.pool1 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.conv2 = freeze_conv(in_channels=64, out_channels=64, kernel_size=motiflen,
                                 padding='same')  # nn.Conv1d(in_channels=64, out_channels=64, kernel_size=motiflen)
        self.pool2 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.conv3 = freeze_conv(in_channels=64, out_channels=64, kernel_size=motiflen,
                                 padding='same')  # nn.Conv1d(in_channels=64, out_channels=64, kernel_size=motiflen)
...
class HEAP_DNA(nn.Module):
    ...
    def forward(self, x, res21, res22, res23, res24):
...
def train(TrainLoader, net, net2, loss_function, optimizer, device, opts, epoch):
    net.train()
    net2.train()
    ...
    for i, data in enumerate(ProgressBar, 0):
        ...
        skip1, skip2, skip3, skip4 = net2(ep)
        # compute output
        outputs = net(inputs, skip1, skip2, skip3, skip4)
...
def eval(Val_loader, net, net2, loss_function, device):
    net.eval()
    running_loss = 0
    correct = 0
    total = 0
...
def _finetune(net, net2, train_loader, val_loader, opts: options, name):
    ...
    optimizer = optim.SGD(itertools.chain(net.parameters(), net2.parameters()), init_lr,
                           weight_decay=wd)
    best_val_loss = 100
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)
    val_errs = []
    for epoch in range(0, epochs + 1):
        train_acc, train_loss, train_AUROC, train_AUPRC = train(train_loader, net, net2, criterion, optimizer, device,
                                                                opts,
                                                                epoch)
        ...
        if epoch % opts.args.eval_epochs == 0:
            ...
            val_acc, val_loss, val_AUROC, val_AUPRC = eval(val_loader, net, net2, criterion, device)
            ...
            state = {
                ...
                'state_dict_net2': net2.module.state_dict() if opts.args.multi_gpu else net2.state_dict()
            }
            if is_best:
                best_auc = val_AUROC
                best_prc = val_AUPRC
...
if __name__ == "__main__":
    for name in human_cell:
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict['state_dict_net2'].items() if k in net2_dict}
        net2_dict.update(pretrained_dict)
        net.load_state_dict(net2_dict)
        net.to(device)
        net2.to(device)

        # train
        train_loader, test_loader, val_loader = get_dataset(name)
        _finetune(net, net2, train_loader, val_loader, opts, name)
        opts.log_settings()
        ...
        net2.to(device)
        net2.load_state_dict(pretrained_dict['state_dict_net2'])
        criterion = nn.CrossEntropyLoss().cuda(device)
        eval(test_loader, net, net2, criterion, device)