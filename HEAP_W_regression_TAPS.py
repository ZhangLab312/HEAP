...
def _finetune(net, net2, train_loader, val_loader, opts: options, name):
    ...
    optimizer = optim.Adam(itertools.chain(net.parameters(), net2.parameters()), init_lr,
                                weight_decay=wd)
    best_val_loss = 100
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)
    val_errs = []
    for epoch in range(0, epochs + 1):
        train_loss, train_pcc = train(train_loader, net, net2, criterion, optimizer, device, opts,
                                                                epoch)
        print(train_loss, train_pcc)
        if epoch % opts.args.eval_epochs == 0:
            # evaluate the performance of initialization
            val_loss, val_pcc = eval(val_loader, net, net2, criterion, device)
...
def get_dataset(species):
    ...
    Y = np.load("./1001/" + name + "/TF_signal.pt.npy")
    Y = np.log2(Y + 1)
    ...
    TestLoader = torch.utils.data.DataLoader(dataset=torch.utils.data.TensorDataset(X_test, Y_test, histone_test),
                                             ...
                                             batch_size=64, shuffle=True, num_workers=0)
    ValidationLoader = torch.utils.data.DataLoader(dataset=torch.utils.data.TensorDataset(X_validation, Y_validation, histone_validation),
