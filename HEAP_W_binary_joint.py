...
def _finetune(net, net2, train_loader, val_loader, opts: options, name):
    ...
    for epoch in range(0, epochs + 1):
        train_acc, train_loss, train_AUROC, train_AUPRC = train(train_loader, net, net2, criterion, optimizer, device, opts,
                                                                epoch)
...
def get_dataset(species):
    ...
    for name in files:
        ...
        Y_1 = np.load("./1001/" + name + "/Y.pt.npy")
        h_1 = np.load("./1001/" + name + "/histone.pt.npy")
        if i == 0:
            X = X_1
            Y = Y_1
            histone = h_1
            ...
            X = np.append(X,X_1,0)
            Y = np.append(Y,Y_1,0)
            histone = np.append(histone,h_1,0)
    l = len(X)
...
if __name__ == "__main__":
    ...
    net2.to(device)
    net.to(device)
    _finetune(net, net2, train_loader, val_loader, opts, 'human')
    opts.log_settings()
    ...
    criterion = nn.CrossEntropyLoss().cuda(device)
    eval(test_loader, net2, net, criterion, device)